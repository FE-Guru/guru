...
const Detail = ({ _id }) => {
  ...
  const appDelete = async () => {
    ...
    if (response.ok) {
      setStatus(
        status.map((applicant) =>
          applicant.email === user.emailID
            ? { ...applicant, status: -1 }
            : applicant
        )
      );
        ...
        <div className="btnWrap">
          ...
          {status.some(
            ...
            >
              ...
              (applicant) =>
                applicant.emailID === user?.emailID && applicant.status === 1
            ) ? (
            ...
            <button
              className="btn tertiary"
              onClick={() => navigate('/job-edit', { state: { _id } })}
            >
              수정하기
            </button>
          ) : status.every(
          ...
          {popupVisible && (
            <SatisfactionModal onClose={closeAlert} type="alert" />
          )}
        </div>
      </section>
      {modalAlert && (
